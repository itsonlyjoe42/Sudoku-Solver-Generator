...
def unique(solve, rowColBoxIndex):
    ...
    for row_col_box in rowColBoxIndex:
        counts = [0] * 9
        last_seen_ix = [0] * 9

        # Count occurences, and keep track of last seen places of each num.
        for ix in row_col_box:
            if len(solve[ix]) == 1:
                num = list(solve[ix])[0]
                counts[num - 1] = 2
            else:
                for num in solve[ix]:
                    counts[num - 1] += 1
                    last_seen_ix[num - 1] = ix

        # If any have been seen exactly once, set the value.
        for num_ix in range(9):
            if counts[num_ix] == 1:
                ix = last_seen_ix[num_ix]
                solve[ix] = [num_ix + 1]
...
# Run all operations until unique sudoku found
